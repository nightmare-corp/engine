pub use std::{borrow::Cow, collections::HashMap, num::NonZeroU32};
pub use ne_gui::{epaint::Primitive, NumExt, PaintCallbackInfo};
pub use type_map::concurrent::TypeMap;
use wgpu::util::DeviceExt;

/// A callback function that can be used to compose an [`ne_gui::PaintCallback`] for custom WGPU
/// rendering.
///
...
pub struct Renderer {
    ...
    /// sampler.
    textures: HashMap<ne_gui::TextureId, (Option<wgpu::Texture>, wgpu::BindGroup)>,
    next_user_texture_id: u64,
    /// Storage for use by [`ne_gui::PaintCallback`]'s that need to store resources such as render
    /// pipelines that must have the lifetime of the renderpass.
    pub paint_callback_resources: TypeMap,
}
//TODO lets take this moment to read up on each renderer component.
//what needs to be done once and what needs to be done just for the ui or meshes...
impl Renderer {
    ...
    pub fn new(
        ...
        let shader = wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Wgsl(Cow::Borrowed(include_str!("../../../../assets/shaders/ui_shader.wgsl"))),
        };
    ...
    pub fn render(
        ...
        paint_jobs: &[ne_gui::epaint::ClippedPrimitive],
    ...
    pub fn update_texture(
        ...
        queue: &wgpu::Queue,
        id: ne_gui::TextureId,
        image_delta: &ne_gui::epaint::ImageDelta,
    ) {
        ...
        if let Some(pos) = image_delta.pos {
            ...
            let filter = match image_delta.filter {
                ne_gui::TextureFilter::Nearest => wgpu::FilterMode::Nearest,
                ne_gui::TextureFilter::Linear => wgpu::FilterMode::Linear,
            };
    ...
    pub fn free_texture(&mut self, id: &ne_gui::TextureId) {
    ...
    /// or [`ne_gui::Context::load_texture`].
    pub fn texture(
        &self,
        id: &ne_gui::TextureId,
    ...
    /// Registers a `wgpu::Texture` with a `ne_gui::TextureId`.
    ///
    ...
    /// Registers a `wgpu::Texture` with an existing `ne_gui::TextureId`.
    ///
    ...
    /// Registers a `wgpu::Texture` with a `ne_gui::TextureId` while also accepting custom
    ...
    pub fn register_native_texture_with_sampler_options(
        ...
        let id = ne_gui::TextureId::User(self.next_user_texture_id);
    ...
    /// Registers a `wgpu::Texture` with an existing `ne_gui::TextureId` while also accepting custom
    /// `wgpu::SamplerDescriptor` options.
    ///
    ...
    pub fn update_buffers(
        ...
        let mut mesh_idx = 0;
        for ne_gui::ClippedPrimitive { primitive, .. } in paint_jobs.iter() {
            match primitive {
                Primitive::Mesh(mesh) => {
...
impl ScissorRect {
    fn new(clip_rect: &ne_gui::Rect, pixels_per_point: f32, target_size: [u32; 2]) -> Self {
